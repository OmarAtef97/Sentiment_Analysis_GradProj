...
data_new = data_balanced.reset_index(drop = True)
data_new
...
data_new['new_text'] = data_new['tweet'].apply(lambda x:' '.join(normalize(x.split())))
data_new = data_new.drop(['tweet'], axis=1)
data_new['sentiment'] = data_new['sentiment'].replace({-1:2})
data_new
...
gnb = GaussianNB().fit(x_train_cv2, y_train) 
gnb_predictions = gnb.predict(x_test_cv2) 


# In[11]:


accuracy_cv = gnb.score(x_train_cv2, y_train)
print(accuracy_cv)
...
accuracy1_cv = gnb.score(x_test_cv2, y_test) 
print (accuracy1_cv) 
...
matrix = classification_report(y_test,gnb_predictions,labels=[0,1,2])
...
gnb2 = GaussianNB().fit(x_train_tv2, y_train) 
gnb_predictions2 = gnb2.predict(x_test_tv2) 


# In[18]:


accuracy1 = gnb2.score(x_train_tv2, y_train)
print(accuracy1)
...
accuracy = gnb2.score(x_test_tv2, y_test) 
print (accuracy) 
...
import matplotlib.pyplot as plt
cm1 = confusion_matrix(y_test, gnb_predictions2)
...
matrix = classification_report(y_test,gnb_predictions2,labels=[0,1,2])
print('Classification Report: \n', matrix)
...
for i, prediction in enumerate(gnb_predictions):
    if prediction == 1:
        pos_count += 1
    elif prediction == 0:
        neu_count += 1
...
train_data_twitter = tv.transform(data_twitter['text'])
...
predictions = gnb.predict(train_data_twitter)
predictions
